...
class AutoConnectDaemon:
    def __init__(self, policy: Optional[RetryPolicy] = None, max_concurrency: int = 5) -> None:
    ...
    async def run_once(self) -> dict[str, bool]:
        ...
        if not powered:
            logger.warning("%d adapter(s) found but none are powered on.", len(adapters))
            return {}

        logger.info("Scanning %d powered adapter(s): %s", len(powered), ", ".join(a.name for a in powered))
        ...
        for adapter in powered:
            ...
            logger.info("%s: %d device(s) known, %d paired+trusted", adapter.name, len(devices), len(eligible))
        ...
        logger.info("Connection pass complete: %d/%d eligible device(s) connected.", succeeded, len(all_results))
        return all_results

    async def _on_dbus_event(self, event_type: str, path: str, interface: str, changed: dict) -> None:
        if interface == ADAPTER_IFACE:
            if event_type in ("added", "properties_changed") and changed.get("Powered") is True:
    ...
    async def run_forever(self) -> None:
        ...
        try:
            while not self._stop_event.is_set():
                ...
                done, pending = await asyncio.wait({rescan_task, stop_task}, return_when=asyncio.FIRST_COMPLETED)
                for task in pending:
                    task.cancel()
